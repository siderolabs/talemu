...
//
// It is a copy of the cluster.DiscoveryServiceController of Talos, stripped of the parts the emulator
// doesn't implement (TLS/proxy dial options).
...
func (ctrl *DiscoveryServiceController) reconcileClients(ctx context.Context, r controller.Runtime, logger *zap.Logger, notifyCh chan<- struct{}) error {
	...
	if err != nil {
		if state.IsNotFoundError(err) {
			// talemu divergence from Talos: on a real node a reset reboots the machine and the whole runtime
			// dies with it, so Talos can leave its clients running here. The emulator keeps running after a
			// reset destroys the machine config, so the clients must be stopped explicitly, or they would keep
			// refreshing the affiliate of a removed machine forever.
	...
	if err != nil {
		if state.IsNotFoundError(err) {
			// same talemu divergence as above: the identity is destroyed on reset, stop the clients with it
			ctrl.stopAllClients(logger)
