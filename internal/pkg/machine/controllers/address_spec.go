...
func (ctrl *AddressSpecController) Run(ctx context.Context, r controller.Runtime, logger *zap.Logger) error {
	...
	for {
		...
		visited := make(map[string]struct{}, len(list.Items))
		...
		for _, res := range list.Items {
			...
			if res.Metadata().Phase() != resource.PhaseTearingDown {
				...
				if err = safe.WriterModify(ctx, r, network.NewAddressStatus(network.NamespaceName, res.Metadata().ID()), func(r *network.AddressStatus) error {
					r.TypedSpec().Address = address.TypedSpec().Address
					r.TypedSpec().LinkName = address.TypedSpec().LinkName
					r.TypedSpec().Family = address.TypedSpec().Family
					r.TypedSpec().Flags = address.TypedSpec().Flags
