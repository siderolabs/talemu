...
.PHONY: sign-image-talemu
sign-image-talemu:  ## Signs the image for talemu. Requires interactive Google authentication.
	@test -n "$$GITHUB_TOKEN" || { \
	...
	@TMP=$$(mktemp -d) && trap 'rm -rf "$$TMP"' EXIT && \
	  ...
	  docker run --rm -p 127.0.0.1:8585:8585 \
	    ...
	    $(REGISTRY)/$(USERNAME)/talemu:$(IMAGE_TAG)
...
.PHONY: sign-image-talemu-infra-provider
sign-image-talemu-infra-provider:  ## Signs the image for talemu-infra-provider. Requires interactive Google authentication.
	@test -n "$$GITHUB_TOKEN" || { \
	  echo 'GITHUB_TOKEN with write:packages scope must be set: gh auth refresh -s write:packages, then GITHUB_TOKEN=$$(gh auth token) make' $@; \
	  exit 1; \
	}
