...
#
# Generated on 2026-07-21T11:24:10Z by kres 359ca55.
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg GOARCH=amd64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=talemu -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /talemu-darwin-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg GOARCH=arm64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=talemu -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /talemu-darwin-arm64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg GOARCH=amd64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=talemu-infra-provider -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /talemu-infra-provider-darwin-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg GOARCH=arm64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=talemu-infra-provider -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /talemu-infra-provider-darwin-arm64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=talemu-infra-provider -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /talemu-infra-provider-linux-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=talemu-infra-provider -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /talemu-infra-provider-linux-arm64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=talemu -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /talemu-linux-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=talemu -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /talemu-linux-arm64
