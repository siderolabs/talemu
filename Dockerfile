...
# RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg govulncheck ./...

# runs unit-tests with race detector
FROM base AS unit-tests-race
WORKDIR /src
ARG TESTPKGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg --mount=type=cache,target=/tmp,id=talemu/tmp CGO_ENABLED=1 go test -v -race -count 1 ${TESTPKGS}

# runs unit-tests
FROM base AS unit-tests-run
WORKDIR /src
ARG TESTPKGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg --mount=type=cache,target=/tmp,id=talemu/tmp go test -v -covermode=atomic -coverprofile=coverage.txt -coverpkg=${TESTPKGS} -count 1 ${TESTPKGS}
...
ARG GO_BUILDFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=talemu/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=talemu/go/pkg go build ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS}" -o /talemu-linux-amd64
